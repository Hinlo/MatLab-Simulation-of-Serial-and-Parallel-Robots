%% LynxMotion Arm Task
% This file uses inverse and forward kinematics to plan and show a task for
% the lynxmotion arm in matlab.

% the chosen task is to draw the vertices of a cuboid in 3d space.

% ***Task Steps***
% 1. define x,y,z positions for 5 or more points that would make a cuboid
% in 3d space.
% 2. use IK to find the q1-5 angle values for the arm at a each point.
% 3. use FK to plot the arm trajectory in 3d space and save the end
% effector points to display the smiley face.
clear all %#ok<*CLALL> %#ok<*SAGROW> 
close all
clc

%% 1. define x,y,z positions that make a smiley face in 3d space.


% TASK: plot a cuboid in 3D space.
points_to_plot = [[0.0 0.0 0.2]; [0.0 0.0 0.1]; [0.15 0.0 0.2]; [0.15 0.0 0.1]; [0.15 0.15 0.1]; [0.15 0.15 0.2]; [0.0 0.15 0.2]; [0.0 0.15 0.1]] ;
% check the plotted points.
figure (1)

for i = 1:size(points_to_plot,1)
    plot3(points_to_plot(i,1), points_to_plot(i,2), points_to_plot(i,3), 'ko-','Linewidth',2)    % plot the first position of the robot's end effector
    hold on
end
axis([ 0 0.4 0 0.4 0 0.4 ])
title('Desired Points') ; xlabel('x (m)') ; ylabel('y (m)') ; zlabel('z (m)');
...
% Run through the list of point
IK_OUTPUT = zeros(size(points_to_plot,1),5); % holding array for output q angles.
...
isequal(round(points_to_plot,2,"decimals"),round(EE,2,"decimals"))
